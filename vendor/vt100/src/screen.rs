...
impl Screen {
    /// Glue `c` onto the cluster before the cursor when a modern terminal
    /// draws the two as one glyph: VS16 (`⚠️`), a ZWJ sequence, a skin tone
    /// or the second half of a flag. The cluster takes the width the whole
    /// string measures, so `⚠️` becomes wide where `⚠` alone is narrow.
    // ponytail: a cluster split across a line wrap is not joined; add when
    // anything prints emoji exactly at the margin and cares.
    fn join_cluster(&mut self, c: char) -> bool {
        ...
        let mut at = crate::grid::Pos {
            row: pos.row,
            col: pos.col.min(size.cols) - 1,
        };
        let Some(prev) = self.grid().drawing_cell(at) else {
            return false;
        };
        if prev.is_wide_continuation() && at.col > 0 {
            at.col -= 1;
        }
        let prev = self.grid().drawing_cell(at).unwrap();
        let text = prev.contents();
        ...
        let joins = match c {
            '\u{fe0f}' => true,
            '\u{1f3fb}'..='\u{1f3ff}' => prev.is_wide(),
            _ if last == '\u{200d}' => true,
            _ if regional(c) => regional(last) && text.chars().count() == 1,
            _ => false,
        };
        if !joins {
            return false;
        }
        let widen = !prev.is_wide()
            && format!("{text}{c}").width() >= 2
            && at.col + 1 < size.cols;
        let cell = self.grid_mut().drawing_cell_mut(at).unwrap();
        cell.append(c);
        if widen {
            cell.widen();
            let next = crate::grid::Pos {
                row: at.row,
                col: at.col + 1,
            };
            let after = crate::grid::Pos {
                row: at.row,
                col: at.col + 2,
            };
            if self.grid().drawing_cell(next).unwrap().is_wide() {
                if let Some(cell) = self.grid_mut().drawing_cell_mut(after) {
                    cell.clear(crate::attrs::Attrs::default());
                }
            }
            let cell = self.grid_mut().drawing_cell_mut(next).unwrap();
            cell.clear(crate::attrs::Attrs::default());
    ...
    pub(crate) fn sgr(
        ...
        loop {
            ...
            match param {
                ...
                [4, n] => {
                    use crate::attrs::UnderlineStyle as U;
                    self.attrs.ulstyle = match n {
                        0 => U::None,
                        2 => U::Double,
                        3 => U::Curly,
                        4 => U::Dotted,
                ...
                [21] => {
                    self.attrs.ulstyle = crate::attrs::UnderlineStyle::Double;
                }
                ...
                [n] if (30..=37).contains(n) => {
                    self.attrs.fgcolor = crate::Color::Idx((*n - 30) as u8);
                }
                [39] => self.attrs.fgcolor = crate::Color::Default,
                [n] if (40..=47).contains(n) => {
                    self.attrs.bgcolor = crate::Color::Idx((*n - 40) as u8);
                }
                [49] => self.attrs.bgcolor = crate::Color::Default,
                [59] => self.attrs.ulcolor = crate::Color::Default,
                [n] if (90..=97).contains(n) => {
                    self.attrs.fgcolor = crate::Color::Idx((*n - 82) as u8);
                }
                [n] if (100..=107).contains(n) => {
                    self.attrs.bgcolor = crate::Color::Idx((*n - 92) as u8);
                }
                [n @ (38 | 48 | 58), ..] => {
                    // An extended colour eats its arguments whether or not
                    // they make sense, so none is read as an attribute.
                    let Some(colour) = extended_colour(param, &mut iter)
                    else {
                        unhandled(self);
                        return;
                    };
                    match n {
                        38 => self.attrs.fgcolor = colour,
                        48 => self.attrs.bgcolor = colour,
                        _ => self.attrs.ulcolor = colour,
                    }
                }
                _ => unhandled(self),
            }
...
/// The colour after `38`, `48` or `58`, in either spelling: colons in one
/// parameter (`38:2::r:g:b`, `38:2:r:g:b`, `38:5:i`) or semicolons across
/// several (`38;2;r;g;b`, `38;5;i`).
fn extended_colour<'a>(
    first: &[u16],
    rest: &mut impl Iterator<Item = &'a [u16]>,
) -> Option<crate::Color> {
    let u8_of = |n: u16| u8::try_from(n).ok();
    let args: Vec<u16> = if first.len() > 1 {
        first[1..].to_vec()
    } else {
        let kind = *rest.next()?.first()?;
        let n = if kind == 2 { 3 } else { 1 };
        let mut args = vec![kind];
        for _ in 0..n {
            args.push(*rest.next()?.first()?);
        }
        args
    };
    match args.as_slice() {
        [2, _, r, g, b] | [2, r, g, b] => {
            Some(crate::Color::Rgb(u8_of(*r)?, u8_of(*g)?, u8_of(*b)?))
        }
        [5, i] => Some(crate::Color::Idx(u8_of(*i)?)),
        _ => None,
    }
}
