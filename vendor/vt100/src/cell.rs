...
// chosen to make the size of the cell struct 40 bytes, which holds the
// longest common emoji ZWJ sequence (a family of four is 25 bytes)
const CONTENT_BYTES: usize = 25;
...
impl Cell {
    ...
    pub(crate) fn append(&mut self, c: char) {
        let len = self.len();
        if len + c.len_utf8() + usize::from(len == 0) > CONTENT_BYTES {
            return;
        }
        if len == 0 {
            self.contents[0] = b' ';
    ...
    /// Mark a narrow cell wide, for a cluster that grew (`⚠` + VS16). The
    /// caller owns the cell to its right.
    ...
    /// Returns how the cell is underlined.
    #[must_use]
    pub fn underline_style(&self) -> crate::UnderlineStyle {
        self.attrs.ulstyle
    }

    /// Returns the underline colour of the cell.
    #[must_use]
    pub fn underline_color(&self) -> crate::Color {
        self.attrs.ulcolor
    }

    /// Returns whether the cell blinks.
    #[must_use]
    ...
    /// Returns whether the cell is struck through.
    #[must_use]
